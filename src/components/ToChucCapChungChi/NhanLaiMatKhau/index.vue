								...
								<div class="card-body">
									<div class="p-5">
										...
										<div class="d-grid gap-2">
                                            ...
                                            <router-link to="/to-chuc-cap-chung-chi/dang-nhap">
...
export default {
    ...
    methods: {
        LayLaiMatKhau() {
	baseRequest
		.post('/to-chuc-cap-chung-chi/lay-lai-mat-khau/' + this.$route.params.hash_reset, this.lay_lai_mat_khau)
		.then((res) => {
			if (res.data.status) {
				toaster.success(res.data.message + ' – bạn sẽ được chuyển về trang đăng nhập trong 5 giây.');
				setTimeout(() => {
					this.$router.push('/to-chuc-cap-chung-chi/dang-nhap');
				}, 5000);
			} else {
